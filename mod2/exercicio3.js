...
if(manha){
  console.log('À partir das 6, exiba "Bom dia!" no console' );
} else if(tarde){
  console.log('À partir das 12, exiba "Boa tarde!"' );
} else {
  console.log('À partir das 18, exiba "Boa noite!"')
}
...
for(let i = 0; i < crazyArray.length; i++){  
  ...
  const numbty = typeofItem === 'number'
  const booleanty = typeofItem === 'boolean'
...
console.log(`O crazyArray tem ${cboolean} booleans, ${numb} números e ${cstring} strings.`)
...
for(let i = 0; i < randomNumbers.length; i++){
  const rand = randomNumbers[i]
  const pares = rand % 2 == 0
  if(pares ){
    par.push(rand)
  } else {
    imp.push(rand)
...
console.log(`Números ímpares: ${imp.join(', ').replace(', 3', ' e 3')}.\n Números pares: ${par.join(', ').replace(', 5', ' e 5')}.`)
